...
**兩台都不是永遠的第一順位。** 依問題性質選工具，不要有預設偏好。這張表要寫進你專案的 `CLAUDE.local.md`，AI 助手才會照著選 —— 可直接貼的版本在 [`claude-md-snippet.md`](claude-md-snippet.md)（做法見第 6.4 節）。

**Neither server is always first choice.** Pick by the question, with no default preference. This split belongs in your project's `CLAUDE.local.md` so your AI assistant follows it — a paste-ready version is in [`claude-md-snippet.md`](claude-md-snippet.md) (section 6.4).
...
腳本可重複執行，所以沒有需要手動貼的步驟。**它會改的「設定檔」只有三個**：`settings.json`、專案的 `CLAUDE.local.md`，以及專案的 `.gitignore`（只附加一行，讓上一個檔案被忽略）。（完整安裝另外會裝 hook 檔案：`~\.claude\hooks\graph-refresh.ps1` 與 `<repo>\.git\hooks\post-commit`，並在覆蓋前留下備份，見第 10 節。）**除非你加 `-SharedAgentDoc`，否則不會寫進共用的 `CLAUDE.md`**（理由見第 6.4 節）。

⚠️ **`.gitignore` 是三個裡面唯一進版控、共用的檔案。** 下面三項保守作法（備份、寫後驗證、回滾）**不涵蓋它**：它只附加一行，而且只在 `git check-ignore` 說還沒被忽略時才加。不想要就用 `-NoAgentDoc`，或先自己把規則寫進 `.git/info/exclude`。

其他兩個檔案的做法是保守的：

The script is idempotent, so there is nothing left to paste. **It patches exactly three CONFIG files**: `settings.json`, the project's `CLAUDE.local.md`, and the project's `.gitignore` (one appended line, so the file above is ignored). (A full install also places the hook files `~\.claude\hooks\graph-refresh.ps1` and `<repo>\.git\hooks\post-commit`, taking a backup before it overwrites either — section 10.) It **never writes into the shared `CLAUDE.md`** unless you pass `-SharedAgentDoc` (why, in section 6.4).

⚠️ **`.gitignore` is the one shared, version-controlled file in that list**, and the three conservatism guarantees below **do not cover it**: it gets one appended line, only when `git check-ignore` says the file is not already ignored. Use `-NoAgentDoc` to avoid it, or put the rule in `.git/info/exclude` yourself first.

For the other two it works conservatively:
...
4. **`settings.json` 裡已存在的值一律保留不覆蓋**，只補缺的，重跑也不會有第二份備份。**這一條只適用 `settings.json`**：`CLAUDE.local.md` 裡我們自己標記之間的區塊，重跑就是就地換新（標記外的內容仍然一個字都不動）。 / **In `settings.json`, never overwrites an existing value**; a re-run adds no second backup. **This guarantee is about `settings.json` only** — the block between our own markers in `CLAUDE.local.md` IS replaced in place on a re-run, while everything outside those markers still survives byte for byte.
...
| `-SelfTest` | 跑腳本自己的測試（29 項斷言：版本解析、答覆判讀、指示檔選擇、主複本閃避），不碰系統 / run the script's own tests (29 assertions), touching nothing |
| `-Repo <repo-root>` | 額外做該專案的步驟（post-commit hook、首次索引、向量、daemon、`CLAUDE.local.md`）/ also run the per-repo steps |
...
| `-NoAgentDoc` | 完全不要寫入任何指示檔（見第 6.4 節）/ do not write the rule anywhere |
| `-SharedAgentDoc` | 改寫進共用、進版控的 `CLAUDE.md`（**不是預設**，見第 6.4 節）/ write into the shared, version-controlled `CLAUDE.md` instead (**not the default**) |
| `-AgentDocName <name>` | 改寫入指定檔案；**明確指定時優先於 `-SharedAgentDoc`** / write into that file; an explicit name **beats `-SharedAgentDoc`** |
...
⚠️ **重新整理只更新索引，永遠不寫進任何指示檔。** `gitnexus analyze` 預設會把自己那段 `<!-- gitnexus:start -->` 區塊 append 進 `CLAUDE.md` 與 `AGENTS.md`；因為這支腳本是每次 commit 都跑，那等於**每次 commit 都偷改專案的共用契約檔**，連你手動刪掉的區塊都會被還原。所以 [`graph-refresh.ps1`](graph-refresh.ps1) 與 [`install.ps1`](install.ps1) 兩處呼叫都固定帶 `--skip-agents-md`。

⚠️ **The refresh only updates the index — it never writes into an instruction file.** By default `gitnexus analyze` appends its own `<!-- gitnexus:start -->` block to `CLAUDE.md` and `AGENTS.md`. This script runs on **every commit**, so that would silently rewrite the project's shared contract each time, restoring a block you had deliberately deleted. Both call sites — in [`graph-refresh.ps1`](graph-refresh.ps1) and [`install.ps1`](install.ps1) — therefore pass `--skip-agents-md`.
...
```powershell
cd <repo-root>
gitnexus analyze --skip-agents-md    # 旗標不能省，見下方 / the flag is not optional, see below
                                     # 需要 taint/PDG 就加 --pdg / add --pdg for taint/PDG
python -m code_review_graph build --repo .
python -m code_review_graph embed --repo .
```

⚠️ **手動跑 `analyze` 時不要漏掉 `--skip-agents-md`。** 沒帶的話，它會把自己那段 `<!-- gitnexus:start -->` 區塊 append 進 `CLAUDE.md` 與 `AGENTS.md` —— 就是本專案兩處呼叫都帶這個旗標要防的那件事。

⚠️ **Do not drop `--skip-agents-md` when you run `analyze` by hand.** Without it, `analyze` appends its own `<!-- gitnexus:start -->` block to `CLAUDE.md` and `AGENTS.md` — exactly what both call sites in this project pass the flag to prevent.
...
**預設寫進 `CLAUDE.local.md`，不是 `CLAUDE.md`。** 這條規則是**個人環境設定**：它點名的 MCP 伺服器只有跑過這支安裝腳本的人才有，引用的 `.claude/skills/gitnexus/` 與 `.gitnexus/run.cjs` 依慣例都不進版控。放進共用、進版控的 `CLAUDE.md`，等於發給每個 clone 的人一份他們照做不了的指示，而且是放在「共同契約」那個檔案裡。Claude Code 原生就會讀 `CLAUDE.local.md`，而且腳本會**確認 git 真的忽略它**（問 `git check-ignore`，需要時才補一行進 `.gitignore`）。

它把規則包在自己的標記裡（`<!-- code-graph-servers:start -->` … `end`）：**重跑會就地更新**，不累積第二份；**標記外的內容一個字都不動**（寫完驗證，不通過就回滾）；檔案不存在就幫你建。規則文字**只有一份來源** —— [`claude-md-snippet.md`](claude-md-snippet.md) 橫線以下的部分（**英文**；正體中文翻譯在 [`claude-md-snippet.zh-TW.md`](claude-md-snippet.zh-TW.md)，參考用，預設不注入，要注入就用 `-SnippetPath` 明確指定），改那裡再重跑即可。真的要寫進共用的 `CLAUDE.md` 就加 `-SharedAgentDoc`；要寫進 `AGENTS.md` 加 `-AgentDocName AGENTS.md`；完全不要它動就加 `-NoAgentDoc`。

⚠️ **舊版預設寫的是 `CLAUDE.md`。** 如果你在改版前裝過，那份區塊還留在共用的 `CLAUDE.md` 裡；新版寫到別的檔案並不會把它移走。腳本偵測到就會**印出警告**，請你自己把 `<!-- code-graph-servers:start -->` … `end` 整段（含標記）刪掉 —— 它不會替你動共用檔案。

**The default target is `CLAUDE.local.md`, not `CLAUDE.md`.** This rule is **personal harness config**: the MCP servers it names exist only for someone who ran this installer, and it cites `.claude/skills/gitnexus/` and `.gitnexus/run.cjs`, both conventionally git-ignored. Put in the shared, version-controlled `CLAUDE.md`, it hands every fresh cloner instructions they cannot follow — inside the file that is supposed to be the shared contract. Claude Code reads `CLAUDE.local.md` natively, and the installer **confirms git actually ignores it** (it asks `git check-ignore` and appends one line to `.gitignore` only if needed).

It wraps the rule in its own markers, so a re-run **updates in place** instead of adding a copy, **nothing outside them changes** (verified, rolled back on failure), and a missing file is created. The rule text has a **single source**: everything below the `---` in [`claude-md-snippet.md`](claude-md-snippet.md), **English only**. The Traditional Chinese translation in [`claude-md-snippet.zh-TW.md`](claude-md-snippet.zh-TW.md) is reference material and is not injected unless you name it with `-SnippetPath`. Use `-SharedAgentDoc` to target the shared `CLAUDE.md` after all, `-AgentDocName AGENTS.md` for that file, `-NoAgentDoc` to skip entirely.

⚠️ **Older versions defaulted to `CLAUDE.md`.** If you installed before this change, that block is still sitting in the shared file, and writing the new copy elsewhere does not remove it. The installer **prints a warning** when it sees one and asks you to delete the `<!-- code-graph-servers:start -->` … `end` block by hand, markers included — it will not edit a shared file for you.
...
- 五個會導致錯答案的前提：未 commit 的程式不在 GitNexus 圖譜裡、非程式碼檔只有 GitNexus 有、語意搜尋需要向量而 daemon 不補向量、`analyze` 失敗也回傳 exit 0、別用 `graph.db` 的 mtime 判斷新鮮度。 / five preconditions that otherwise produce wrong answers.
- **唯一刻意重疊的一題**：C++ 跨檔呼叫者走 GitNexus `impact`（對 `.hpp` 的 uid，`direction: 'upstream'`）。 / the one deliberate overlap: C++ cross-translation-unit callers.
- **圖譜可信到哪裡**：兩台都少報過同一個 C++ 函式的呼叫者，所以會左右動作的呼叫者清單要用 `git grep` 加陽性對照再確認一次。 / what the graphs are authoritative for, and the `git grep` fallback.
- **手動跑 `analyze` 要帶 `--skip-agents-md`**，否則它會改寫共用的 `CLAUDE.md`。 / a hand-run `analyze` needs the flag.
...
### 6.5 `.gitignore`：索引資料夾不用管，`CLAUDE.local.md` 由腳本處理 | `.gitignore`: index folders need nothing, `CLAUDE.local.md` is handled

兩個工具都會在自己的資料夾裡放一個內容為 `*` 的 `.gitignore`，自我忽略。`git status` 看不到它們，**不要再手動加規則**。

`CLAUDE.local.md` 是唯一的例外，而且腳本已經替你處理：它問 `git check-ignore`，只有在 git 確實還沒忽略時才補一行。之所以問 git 而不是讀 `.gitignore`，是因為規則也可能寫在 `.git/info/exclude` 或全域 excludesFile —— 用文字搜尋會漏掉那兩處，然後補上一行重複的。
...
`CLAUDE.local.md` is the one exception, and the installer already handles it: it asks `git check-ignore` and appends a line only when git does not already ignore the file. It asks git rather than reading `.gitignore` because the rule may equally live in `.git/info/exclude` or a global excludesFile — a text search would miss both and append a duplicate.
...
```powershell
gitnexus analyze --repair-fts --skip-agents-md
```
...
再手動清掉：`%USERPROFILE%\.claude\hooks\graph-refresh.ps1`、`<repo-root>\.git\hooks\post-commit`、`settings.json` 裡的 `SessionStart` 條目與 `env` 那幾行、以及專案 `CLAUDE.local.md` 裡 `<!-- code-graph-servers:start/end -->` 之間的區塊（舊版裝的會在 `CLAUDE.md`）。安裝時留下的 `*.local-<時間戳>` 備份也可以一併刪掉。

Then remove by hand: `%USERPROFILE%\.claude\hooks\graph-refresh.ps1`, `<repo-root>\.git\hooks\post-commit`, the `SessionStart` entry and `env` lines in `settings.json`, and the `<!-- code-graph-servers:start/end -->` block in the project's `CLAUDE.local.md` (an older install put it in `CLAUDE.md`). Any `*.local-<timestamp>` backups the installer left behind can go too.
...
| [`claude-md-snippet.md`](claude-md-snippet.md) | 貼進你專案 `CLAUDE.local.md` 的分工規則，**英文，安裝腳本注入的就是這份**（見第 6.4 節）/ the usage rule for your project's `CLAUDE.local.md` — **English, and the one the installer injects** (section 6.4) |
| [`claude-md-snippet.zh-TW.md`](claude-md-snippet.zh-TW.md) | 同一條規則的正體中文翻譯，**參考用，預設不注入** / the same rule in Traditional Chinese, **reference only, not injected by default** |
| `README.md` | 本文件（中英雙語）/ this document (bilingual) |

### 「主複本」是有人在管的 | The master-copy claim is enforced

`graph-refresh.ps1` 與 `post-commit` 的檔頭都寫著「主複本在 dev-workstation repo」。以前那只是一句話：`install.ps1` 用 `Copy-Item -Force` 直接蓋掉安裝好的複本，所以**有人在安裝端修好的東西，下一次安裝就被無聲還原**，而且沒有任何人被告知。這件事真的發生過一次。

現在安裝前先比對 SHA-256：內容相同就是 no-op（所以連跑兩次，第二次會回報 already current）；內容不同就先把安裝端那份存成 `<檔名>.local-<時間戳>`，**印出警告說明它被換掉了、備份在哪裡**，再覆蓋。主複本仍然贏 —— 這不是合併機制 —— 但你會知道自己剛剛失去了什麼，以及該把那個修正補回哪裡。

Both `graph-refresh.ps1` and `post-commit` carry a header naming this repo as the master copy. That used to be only a sentence: `install.ps1` overwrote the installed copy with `Copy-Item -Force`, so **a fix applied at the installed end was silently reverted by the next install**, with nobody told. That happened once, for real.

The installer now compares SHA-256 first. Identical content is a no-op, which is why a second run in a row reports `already current`. Different content is saved to `<name>.local-<timestamp>`, a **warning prints saying what was replaced and where the backup is**, and only then is it overwritten. The master still wins — this is deliberately not a merge mechanism — but you learn what you just lost and where the fix belongs.
