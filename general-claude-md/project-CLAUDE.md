...
<!-- FILL: (or delete) WHEN a commit happens. ⚠ Deleting this section is not "no policy":
     it leaves the harness default in force, which is that the agent commits and pushes
     only when the owner asks, every time. A push is outward-facing, so it is confirmed
     first unless the project durably authorises it — and a filled slot here IS that
     authorisation. Name the ONE checkpoint that triggers a commit plus a push, and
     nothing as vague as "regularly" — e.g. "Commit and push every time a sub-task is
     marked done in the task folder's progress record." Other checkpoints that work: every
     green full-suite run · every completed refute-fix wave (`unattended-work` §3). This
     slot governs cadence DURING the work; the obligation to
     have everything committed and pushed at handback already lives in `unattended-work`
     §14. The user-scope git rules govern HOW (check the branch before every commit,
     message via `git commit -F`, screen for credentials, push in the same breath) — this
     slot only says WHEN. -->
